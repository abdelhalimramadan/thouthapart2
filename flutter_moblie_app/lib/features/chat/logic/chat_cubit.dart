...
class ChatCubit extends Cubit<ChatState> {
  ...
  void _processResponse(
    ...
    if (response.result != null) {
      final category = (response.result!['category'] ?? response.result!['category_en'])?.toString();
    ...
    if (q != null && q.id != null && q.text != null) {
      flowItems.add(FlowItem(type: FlowType.question, text: q.text!, question: q));
      activeQuestionId = q.id;
       ...
       flowItems.add(FlowItem(type: FlowType.question, text: flatQ.text!, question: flatQ));
  ...
  int? getCategoryIdByName(String rawCategory, List<Map<String, dynamic>> categories) {
    try {
  ...
  Future<void> sendChatMessage(String message) async {
    ...
    emit(currentState.copyWith(chatHistory: chatHistory, isActionLoading: true));
    ...
    chatHistory.removeWhere((m) => m.role == ChatRole.bot && m.text == 'يفكر.....');
    ...
    if (result['success'] == true) {
      ...
      chatHistory.add(const ChatItem(role: ChatRole.bot, text: 'عذراً، حدث خطأ في الاتصال.'));
      emit(currentState.copyWith(chatHistory: chatHistory, isActionLoading: false));
    }
  }
